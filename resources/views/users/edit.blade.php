...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification des données utilisateur') }}
        </h2>
    </x-slot>
    <div class="px-6 py-12">
        <div class="px-6 py-6 mx-auto max-w-xl bg-white shadow-lg rounded-lg">
            <h3 class="pb-4 text-lg font-bold text-gray-900 border-b-2 border-sky-500 uppercase">Modification des données utilisateur
            </h3>
            <form class="space-y-6" action={{route('users.update', $user->id)}} method="POST"
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name"
                        ...
                        placeholder="Entrer le nom complet" value="{{$user->name }}" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="email" id="email"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 placeholder-gray-400"
                        placeholder="Entrer le adresse email" value="{{$user->email }}" required>
                    @error('email')
                    <span class="text-sm text-red-500">* {{$message}}</span>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <label for="dropdownCheckboxButton" class="block mb-2 text-sm font-medium text-gray-900">Categories
                        accessibles</label>
                    <button id="dropdownCheckboxButton" data-dropdown-toggle="dropdownDefaultCheckbox"
                        ...
                        type="button">Choisissez des Categories <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true"
                ...
                <div>
                    ...
                    <input type="password" name="password" id="password"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 placeholder-gray-400"
                        placeholder="Entrer le mot de passe" value="{{$user->password }}" required
                ...
                <!-- Name -->
                <div>
                    <label for="password_confirmation" class="block mb-2 text-sm font-medium text-gray-900">Confirmez le
                        mot de
                        passe</label>
