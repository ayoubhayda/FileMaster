    ...
    <div class="px-6 py-6 mx-auto max-w-xl bg-white shadow-lg rounded-lg">
      ...
      <form class="space-y-6" action={{route('users.store')}} method="POST" enctype="multipart/form-data">
        ...
        <div>
          ...
          <label for="dropdownCheckboxButton" class="block mb-2 text-sm font-medium text-gray-900">Categories accessibles</label>
          ...
          <div id="dropdownDefaultCheckbox"
            class="z-10 hidden overflow-y-auto max-h-44 w-48 bg-white divide-y divide-gray-100 rounded-lg shadow">
            <ul class="p-3 space-y-3 overflow-y-auto text-sm text-gray-700" aria-labelledby="dropdownCheckboxButton">
                @foreach ($categories as $category)
                        <li>
                            ...
                            <label for="checkbox-item-{{$category->id}}" class="ml-2 text-sm font-medium text-gray-900">{{$category->name}}</label>
                        </div>
                    </li>
                 @endforeach
            </ul>
